...
async def capture_chat_id(token: str | None = None) -> Any:
    """Capture a Discord channel ID and return it.

    For Discord, we capture channel ID instead of chat_id.
    """
    ...
    if token is None:
        console.print("[red]error:[/] discord bot token is required")
        return None

    result = await _validate_discord_token(token)
    if result is None:
        console.print("[red]error:[/] failed to validate token")
        return None

    bot_id, bot_name = result
    console.print(f"connected to {bot_name}")

    # Prompt for channel ID
    channel_id_str = await questionary.text(
        "enter channel ID (right-click channel → Copy Channel ID):"
    ).ask_async()
    ...
    try:
        ...
        return ChatInfo(chat_id=channel_id, chat_title=f"Channel {channel_id}")
    except ValueError:
        console.print("[red]error:[/] invalid channel ID")
        return None


def debug_onboarding_paths() -> None:
    """Print debug info about onboarding paths."""
    console = Console()

    paths = [
        ("config file", HOME_CONFIG_PATH),
    ]

    table = Table(title="onboarding paths", box=box.SIMPLE)
    ...
    for path_type, path in paths:
        ...
        table.add_row(path_type, _display_path(path), exists)
...
async def interactive_setup(*, force: bool) -> bool:
    ...
    with suppress_logs():
        panel = Panel(
            ...
            title="welcome to yee88-discord!",
            border_style="blue",
            padding=(1, 2),
            expand=False,
        )
        ...
        # Optional: prompt for channel ID
        channel_id: int | None = None
        use_channel = await _confirm(
            "set a default channel for startup messages?", default=False
        )
        if use_channel:
            channel_id_str = await questionary.text(
                "enter channel ID (right-click channel → Copy Channel ID):"
            ...
            if channel_id_str:
                try:
                    ...
                    console.print("  invalid channel ID, skipping")
        ...
        if channel_id is not None:
            discord_section["channel_id"] = channel_id

        write_config(merged, config_path)
        console.print(f"  config saved to {_display_path(config_path)}")

        done_panel = Panel(
            "setup complete. run 'yee88' to start yee88-discord!\n\n"
            "tip: mention the bot in a channel to interact with it.",
            border_style="green",
            padding=(1, 2),
            expand=False,
        )
