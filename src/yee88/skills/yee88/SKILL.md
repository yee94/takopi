...
| 用户说 | 必须执行的命令 |
|--------|----------------|
| "5分钟后提醒我..." | `yee88 cron add reminder "+5m" "提醒内容" --project <alias> -o` |
| "30分钟后提醒我..." | `yee88 cron add reminder "+30m" "提醒内容" --project <alias> -o` |
| "2小时后提醒我..." | `yee88 cron add reminder "+2h" "提醒内容" --project <alias> -o` |
| "明天提醒我..." | `yee88 cron add reminder "+1d" "提醒内容" --project <alias> -o` |
| "每天早上9点提醒我..." | `yee88 cron add daily "0 9 * * *" "提醒内容" --project <alias>` |
| "每周一提醒我..." | `yee88 cron add weekly "0 9 * * 1" "提醒内容" --project <alias>` |
...
```bash
yee88 cron add <id> "<时间>" "<消息>" --project <alias> -o
```
...
- `--project <alias>`: 项目别名（如 takopi, myproject），通过 `yee88 config list | grep projects` 查看已注册项目
- `-o`: 一次性任务（执行后自动删除）

**正确示例：**
```bash
# 用户说："5分钟后提醒我该健身了"
yee88 cron add reminder "+5m" "该健身了" --project takopi -o

# 用户说："30分钟后提醒我开会"
yee88 cron add meeting "+30m" "开会时间到" --project takopi -o

# 用户说："1分钟后提醒我查天气"
# ✅ 正确：只设置提醒，不查天气
yee88 cron add weather "+1m" "查天气" --project takopi -o

# ❌ 错误：立即查天气并把结果写进 message
# yee88 cron add weather "+1m" "杭州今天晴，15度..." --project takopi -o
```
...
- `message`: 推送消息内容
- `--project`: 项目别名（必填，通过 `yee88 config list | grep projects` 查看已注册项目）
...
| `yee88 cron add <id> <time> <msg> --project <alias> -o` | 添加一次性任务 (-o = --one-time) |
